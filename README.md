...
To prepare this project to run, you'll need to have **NodeJs** 14.17.6 or up installed in your computer. I suggest to use NVM in order to manage different versions easily without incompatibility dependencies. To check the node version via nvm use `nvm ls` once it specify the version change it with `nvm use (version)`.
...
There is a `./src/boilerplate/data/data.js`, here you could handle the copy for your email.For example:
